...
```{r}
shell_length_neb <- function(initial_length, CHL, TEMP, days, MTA = 0.76, ECS = 0.161, a = 2.767, b = 0.827,NIRREMORG = 0, EREM = 0, WE = 1, WS = 1, MNEA = 1350) {
  selorg = (CHL*50)/0.38
  nirselorg = (-0.33+(4.11*selorg)) #check table 2 --> TEF and (WE/WS)^0.62
  nea = (((nirselorg*23.5)+(NIRREMORG*0.15*EREM))*0.8*24)
  cr = 0.32+(0.323*TEMP)+(-.011*(TEMP)^2)
...
shell_length_neb(1, 0.004, 12, 600)
...
X_lo <- seq(0, 12.5, length.out=100)
...
ggplot()+
  ...
  theme_minimal() +
  geom_text(aes(x = 12.7,y = 6, label= "Average choice (no info)", hjust = 0), size = 3, color = "gray34")+
  geom_vline(aes(xintercept = 12.5),linetype = "dashed", color = "dodgerblue1")+
  labs(x = "Choice (X)",
       y = "Profits")
